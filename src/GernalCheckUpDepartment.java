...
public abstract class GernalCheckUpDepartment implements ReceptionistR ,PatientPayment
{
	...
	public void Login(int Password) throws InterruptedException
	{
		...
		while(f==false)
		{
			if(Password==1234 || Password==Forget)
			{
				...
				System.out.println("Welcome Back Admin (Gernal CheckUp Department)");
	...
	public void insertNew ()throws IOException
	{
	    ...
	    Fout=new FileWriter("GernalCheckUpDepartment.txt" ,false);
	...
	public void Hview () throws IOException
	{
		...
		Fin=new FileReader("GernalCheckUpDepartment.txt");
	    ...
	    while(Obj.hasNext())
	    {
	    	...
	    	System.out.println("Head    Of      GernalCheckUp Department is = "+HeadName 
	    			        +"\nBlock Number Of GernalCheckUp Department is = "+BlockNumber+"\n");
	    }
	    Fin.close();
	    Bin.close();
	    Obj.close();
...
class Gernalreceptionist extends GernalCheckUpDepartment
{
	...
	public void RGDinsertNew ()throws IOException
	{
		int []doctorID=new int[13];
		int []doctorTime=new int[13];
		FileWriter Fout;
		BufferedWriter Bout = null;
	    Fout=new FileWriter("GernalCheckUpReceptionist.txt" ,true);
	    Bout=new BufferedWriter(Fout);
	    for(int i=0 ; i<=ReceptionistR.TotalDoctors ; i++)
		{
	...
	public void RGDview () throws IOException
	{
		int [] DoctorID =new int[13];
		int [] DoctorTime=new int[13];
		...
		FileReader Fin;
		BufferedReader Bin = null;
		Fin=new FileReader("GernalCheckUpReceptionist.txt");
	    Bin=new BufferedReader(Fin);
		Scanner Obj=new Scanner(Bin);
	    while(Obj.hasNext())
	    {
	...
	void RGDSearch(int FindID)throws IOException
	{
		int [] DoctorID =new int[13];
		int [] DoctorTime=new int[13];
		String [] DoctorName=new String[13];
	...
	void RGDDelete(int Delete)throws IOException
	{
		int [] DoctorID =new int[13];
		int [] DoctorTime=new int[13];
		String [] DoctorName=new String[13];
		int i=0;
	    File fileA=new File("GernalCheckUpReceptionist.txt");
	...
	void RGDUpdate(int Find)throws IOException
	{
		int [] DoctorID =new int[13];
		int [] DoctorTime=new int[13];
		String [] DoctorName=new String[13];
		int i=0;
	    File file=new File("GernalCheckUpReceptionist.txt");
	...
	public void GDFindDoctor() throws IOException, InterruptedException
	{
		...
		if(DoctorTime[i]==Time)
	    {
			...
			Payment.PatientPament(PatientPayment.GernalDoctorPayment);
	    }
	    }
	    Fin.close();
	    Bin.close();
